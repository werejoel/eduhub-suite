...
function TeacherDashboard() {
  ...
  const { data: marks = [], isLoading: marksLoading, isError: marksError, error: marksErrorObj } = useMarks();
  const { data: attendance = [], isLoading: attendanceLoading, isError: attendanceError, error: attendanceErrorObj } = useAttendance();
  const { data: duties = [], isLoading: dutiesLoading, isError: dutiesError, error: dutiesErrorObj } = useDutiesByTeacher(user?.id || "");
  ...
  return (
    <DashboardLayout>
      ...
      {!loading && teacherClasses.length === 0 && (
        ...
        >
          <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-amber-900 mb-2">No Classes Assigned</h2>
            <p className="text-amber-800 mb-6">You don't have any classes assigned yet. Contact your administrator to get assigned to classes.</p>
            <button
              onClick={() => window.location.reload()}
              className="inline-block px-6 py-2 bg-amber-600 text-white rounded-lg font-medium hover:bg-amber-700 transition-colors"
            >
              Refresh Page
            </button>
          </div>
      ...
      {/* Stats Grid - Only show if has classes */}
      ...
      {/* Duties Assigned - Only show if has classes */}
      {teacherClasses.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Charts & Classes - Only show if has classes */}
      {teacherClasses.length > 0 && (
